...
def eval_all(expressions, env):
    ...
    if expressions== nil:
        return None
    exprssns = expressions.rest
    last = scheme_eval(expressions.first, env)
    while exprssns:
        last = scheme_eval(exprssns.first,env)
        exprssns= exprssns.rest
    return last
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    bln = True
    while scheme_truep(bln) and not (expressions is nil):
        bln = scheme_eval(expressions.first, env)
...
def do_or_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    bln = False
    while scheme_falsep(bln) and not (expressions is nil):
        bln = scheme_eval(expressions.first, env)
        expressions= expressions.rest
    return bln
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            return eval_all(clause.rest, env) or test
            # END PROBLEM 13
        expressions = expressions.rest
